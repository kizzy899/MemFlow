...
脚本不新增公共 HTTP API、持久化字段或业务状态流转。应用启动时仍由现有 lifespan 创建目录、初始化 SQLite 表并执行迁移。非 `-Check` 启动会先调用 `scripts\start_chrome_cdp.ps1 -Port <ChromeCdpPort>`；该脚本复用 `data\chrome-cdp-profile`，并且在端口已监听时直接返回。
...
- 非 `-Check` 启动：验证日志先出现 Chrome CDP 启动或复用提示，再出现 MemFlow 服务地址。
