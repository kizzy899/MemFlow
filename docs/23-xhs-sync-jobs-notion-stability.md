...
任务快照字段：`task_id`、`status`、`phase`、`message`、`requested`、`fetched`、`processed`、`success`、`failed`、`current_index`、`current_title`、`last_error`、`started_at`、`updated_at`、`finished_at`。

状态流转：`idle -> fetching -> processing -> success|failed`。同一时刻只允许一个收藏任务；重复启动返回 409。抓取异常直接进入 `failed`；单条整理失败累计到 `failed`，批次结束状态也为 `failed`，本地条目继续保留其原有失败信息。
