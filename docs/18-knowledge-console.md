...
首页左栏依次提供小红书授权、收藏读取参数和 Notion 配置；`hot.md` 入口移动到右侧“待整理收件箱”卡片右上角，位于待整理表单与队列内容上方。收藏读取数量为 1–100，直接调用 `POST /api/xhs/sync` 并显示执行状态。账号页只负责 Chrome 连接和会话管理，不再重复展示收藏读取表单。
