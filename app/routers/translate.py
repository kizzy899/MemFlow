...
from app.schemas.content import TranslateRequest, TranslateResponse, TranslationTaskRequest, TranslationTaskStatus
...
def submit_translation(
    ...
    try:
        if payload.type == "url":
            item = container.translation_service.translate_url(db, payload.content)
        else:
...
@router.post("/translate/tasks", response_model=TranslationTaskStatus)
def start_translation_task(
    payload: TranslationTaskRequest, request: Request, db: Session = Depends(get_db)
) -> TranslationTaskStatus:
    container: ServiceContainer = request.app.state.container
    url = str(payload.url)
    existing = container.translation_service.find_existing_translation(db, url)
    if existing:
        return TranslationTaskStatus(**container.translation_task_manager.state_for_existing(existing))
    return TranslationTaskStatus(**container.translation_task_manager.start(url))


@router.get("/translate/tasks/status", response_model=TranslationTaskStatus)
def translation_task_status(request: Request) -> TranslationTaskStatus:
    container: ServiceContainer = request.app.state.container
    return TranslationTaskStatus(**container.translation_task_manager.status())
